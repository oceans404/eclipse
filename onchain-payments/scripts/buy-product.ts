...
if (wallets.length === 1) {
  console.log("⚠️  Using same wallet for buyer and creator. Set BUYER_PRIVATE_KEY for different buyer.\n");
}
...
const [price, creator, contentId, exists] = await paymentService.read.getProduct([productId]);
...
const hasPaid = await paymentService.read.hasPaid([wallet.account.address, productId]);
if (hasPaid) {
...
console.log("\n💳 Purchasing product...");
try {
  const buyTx = await paymentService.write.payForProduct(
    [productId],
    { account: wallet.account }
  );
