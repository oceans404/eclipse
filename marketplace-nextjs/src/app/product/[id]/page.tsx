...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  console.log('isNaN(productId):', isNaN(productId));
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="text-2xl font-semibold text-gray-800 mb-2">Invalid Product ID</h3>
          <p className="text-gray-600 mb-6">The product ID must be a valid number.</p>
          <button
            onClick={() => router.push('/')}
  ...
  if (!product) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-purple-50 p-8 flex items-center justify-center">
        <div className="text-center">
          <div className="text-6xl mb-4">❓</div>
          <h3 className="text-2xl font-semibold text-gray-800 mb-2">Product not found</h3>
  ...
  const totalRevenue = data?.ProductPaymentService_PaymentReceived?.reduce((sum: string, payment: any) => {
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-8 text-white">
            <div className="flex items-center space-x-2 mb-4">
              <Package size={24} />
              <span className="text-lg font-medium">Product #{product.productId}</span>
            </div>
            <h1 className="text-4xl font-bold mb-2 break-all">{product.contentId}</h1>
            <p className="text-blue-100">Premium data content stored securely with Nillion</p>
          </div>
          ...
          <div className="p-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-100">
                  <h3 className="text-lg font-semibold text-green-800 mb-4">Sales Statistics</h3>
                ...
                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100">
                  <h3 className="text-lg font-semibold text-blue-800 mb-4">Product Details</h3>
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm text-blue-600 font-medium">Content ID</p>
                      <p className="text-blue-900 break-all font-mono text-sm">{product.contentId}</p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-600 font-medium">Product ID</p>
                      <p className="text-blue-900 font-mono">{product.productId}</p>
                    </div>
                    <div>
                      <p className="text-sm text-blue-600 font-medium">Last Updated</p>
                      <p className="text-blue-900">{formatDate(product.lastUpdatedAt)}</p>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  {priceHistory.map((entry: any, index: number) => (
                    <div key={index} className="flex justify-between items-center bg-white rounded-xl p-4">
                      <div className="flex items-center space-x-3">
                        <div className={`w-3 h-3 rounded-full ${entry.type === 'created' ? 'bg-blue-500' : 'bg-orange-500'}`}></div>
                        <div>
                          <p className="text-sm font-medium text-gray-900">
                            {entry.type === 'created' ? 'Product Created' : 'Price Updated'}
                          </p>
            ...
            {data?.ProductPaymentService_PaymentReceived && data.ProductPaymentService_PaymentReceived.length > 0 && (
              <div className="mt-8 bg-gray-50 rounded-2xl p-6">
                <h3 className="text-lg font-semibold mb-4">Recent Purchases</h3>
                <div className="space-y-3">
                  {data.ProductPaymentService_PaymentReceived.slice(0, 5).map((payment: any, index: number) => (
                    <a
                      key={index}
